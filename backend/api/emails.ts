...
emailsApi.get('/', async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to retrieve emails', emails: [], stats: { total: 0, unique: 0, signups: 0, logins: 0, lastUpdated: null } }, 500);
  }
});
...
emailsApi.get('/export', async (c) => {
  try {
    ...
    const content = format === 'csv' ? await emailStorageService.exportEmailsAsCSV() : await emailStorageService.exportEmailsAsText();
    return c.text(content, 200, { 'Content-Type': format === 'csv' ? 'text/csv' : 'text/plain', 'Content-Disposition': ttachment; filename="emails-." });
  } catch (error) {
